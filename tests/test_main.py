...
def test_main_success_path(
    mock_service_cls, mock_setup_logging, mock_load_config, basic_app_settings
):
...
@patch(
    "finetune_embedding.main.load_and_validate_config",
    side_effect=ConfigurationError("Bad config"),
)
@patch("finetune_embedding.main.setup_logging")
@patch("finetune_embedding.main.FineTuningService")
def test_main_config_error(
    mock_service_cls, mock_setup_logging, mock_load_config, caplog
):
...
def test_main_pipeline_error(
    mock_service_cls, mock_setup_logging, mock_load_config, basic_app_settings, caplog
):
...
def test_main_unexpected_error(
    ...
    mock_load_config.return_value = basic_app_settings
    mock_service_instance = mock_service_cls.return_value
    mock_service_instance.run_pipeline.side_effect = Exception(
    ...
    assert exit_code == 1
    assert (
        "An unexpected critical error occurred: Something totally unexpected"
        in caplog.text
    )
