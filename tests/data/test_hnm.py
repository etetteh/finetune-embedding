...
def pair_dataset_container() -> DatasetContainer:
    ...
    return DatasetContainer(train=train_ds, eval_dataset=eval_ds)
...
def test_hnm_skips_non_pair_format(
    triplet_dataset_container,
    mock_sentence_transformer_instance,
    default_hnm_config,
    triplet_dataset_config,
):
...
def test_hnm_runs_for_pair_format(
    mocker,
    pair_dataset_container,
    mock_sentence_transformer_instance,
    default_hnm_config,
    pair_dataset_config,
):
    ...
    mock_train_result = pair_dataset_container.train_dataset.add_column(
        "negative", ["neg_train"] * len(pair_dataset_container.train_dataset)
    )
    mock_eval_result = pair_dataset_container.eval_dataset.add_column(
        "negative", ["neg_eval"] * len(pair_dataset_container.eval_dataset)
    )
...
# This test PASSED before, should still pass
def test_hnm_error_if_no_model(
    pair_dataset_container, default_hnm_config, pair_dataset_config
):
...
def test_hnm_error_if_no_train_data(
    mock_sentence_transformer_instance, default_hnm_config, pair_dataset_config
):
...
def test_hnm_error_if_mining_fails(
    mocker,
    pair_dataset_container,
    mock_sentence_transformer_instance,
    default_hnm_config,
