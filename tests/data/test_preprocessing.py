...
def sample_json_file_factory(tmp_path_factory):
    ...
    def _factory(filename="sample_map.json", content=None):
        if content is None:
            content = {"orig_a": "anchor"}  # Create default dict here
    ...
    return DatasetDict(
        {
            "train": Dataset.from_dict(
                {
                    "sentence1": [f"tr{i}a" for i in range(100)],
                    "sentence2": [f"tr{i}b" for i in range(100)],
                }
            ),
            "validation": Dataset.from_dict(
                {
                    "sentence1": [f"va{i}a" for i in range(30)],
                    "sentence2": [f"va{i}b" for i in range(30)],
                }
            ),
            "test": Dataset.from_dict(
                {
                    "sentence1": [f"te{i}a" for i in range(20)],
                    "sentence2": [f"te{i}b" for i in range(20)],
                }
            ),
            "dev": Dataset.from_dict(
                {
                    "sentence1": [f"de{i}a" for i in range(10)],
                    "sentence2": [f"de{i}b" for i in range(10)],
                }
            ),
        }
    )
...
def test_get_expected_columns(data_format, expected_keys):
    cols = get_expected_columns(data_format)
...
def test_parse_column_rename_map_invalid_json_string():
    ...
    with pytest.raises(
        ConfigurationError,
        match="'{.*}' is neither valid JSON, a dict, nor an existing file.",
    ):
...
def test_parse_column_rename_map_file_not_found():
    ...
    with pytest.raises(
        ConfigurationError,
        match="'non_existent_map.json' is neither valid JSON, a dict, nor an existing file.",
    ):
...
def test_parse_column_rename_map_invalid_type():
    with pytest.raises(
        ConfigurationError, match="Unsupported type for column_rename_map"
    ):
...
def test_parse_column_rename_map_invalid_content():
    ...
    with pytest.raises(
        ConfigurationError, match="Keys/values in column_rename_map must be strings"
    ):
...
def test_select_limit_eval_auto_detect(full_dataset_dict, pair_format_config, caplog):
    ...
    assert (
        "Automatically selected existing split 'dev'" in caplog.text
...
def test_select_limit_empty_train_fails(pair_format_config):
    empty_dict = DatasetDict(
        {"train": Dataset.from_dict({"sentence1": [], "sentence2": []})}
    )
...
def test_select_limit_missing_required_cols(pair_format_config):
    ...
    bad_dict = DatasetDict(
        {"train": Dataset.from_dict({"id": [1, 2]})}
    )  # Missing required cols
    with pytest.raises(
        DataLoadingError, match="Train dataset missing required columns"
    ):
