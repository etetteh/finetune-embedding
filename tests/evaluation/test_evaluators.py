...
def mock_dataset_config() -> DatasetConfig:
    ...
    return DatasetConfig(
        dataset_format="pair", eval_split="eval", test_split="test"
    )  # Minimal valid
...
def test_create_evaluator_triplet(
    mock_triplet_eval, triplet_dataset, mock_dataset_config
):
    """Test creating TripletEvaluator."""
    mock_dataset_config.dataset_format = "triplet"  # Ensure format matches
    evaluator = create_evaluator(
        triplet_dataset, "triplet", mock_dataset_config, 16, "eval"
    )
    assert isinstance(
        evaluator, MagicMock
    )  # Check if the mocked class instance was returned
...
def test_create_evaluator_pair_score(
    mock_sim_eval, pair_score_dataset, mock_dataset_config
):
    """Test creating EmbeddingSimilarityEvaluator."""
    mock_dataset_config.dataset_format = "pair-score"
    evaluator = create_evaluator(
        pair_score_dataset, "pair-score", mock_dataset_config, 16, "test"
    )
...
def test_create_evaluator_pair_class(
    mock_bin_eval, pair_class_dataset, mock_dataset_config
):
    """Test creating BinaryClassificationEvaluator with filtering."""
    mock_dataset_config.dataset_format = "pair-class"
    evaluator = create_evaluator(
        pair_class_dataset, "pair-class", mock_dataset_config, 16, "eval"
    )
    assert isinstance(evaluator, MagicMock)
