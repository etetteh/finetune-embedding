...
def test_run_evaluation_success_float(
    mock_evaluator_instance, mock_sentence_transformer_instance, temp_output_dir
):
    ...
    with patch("builtins.open", mock_open()) as mock_file:
        run_evaluation(
            mock_evaluator_instance,
            mock_sentence_transformer_instance,
            str(temp_output_dir),
        )

    mock_evaluator_instance.assert_called_once_with(
        mock_sentence_transformer_instance, output_path=str(temp_output_dir)
    )
    # Check if results file was opened correctly
    mock_file.assert_called_once_with(
        os.path.join(temp_output_dir, "eval_test-split_results.json"), "w"
    )
    handle = mock_file()
...
def test_run_evaluation_success_dict(
    mock_evaluator_instance, mock_sentence_transformer_instance, temp_output_dir
):
...
def test_run_evaluation_no_evaluator(
    mock_sentence_transformer_instance, temp_output_dir
):
...
def test_run_evaluation_save_fails(
    mock_evaluator_instance, mock_sentence_transformer_instance, temp_output_dir, caplog
):
...
def test_run_evaluation_eval_fails(
    mock_evaluator_instance, mock_sentence_transformer_instance, temp_output_dir, caplog
):
    ...
    # --- END FIX ---
    assert (
        "Internal evaluator error" in caplog.text
    )  # Also check the specific error is logged
