...
from finetune_embedding.config.settings import (
    AppSettings, ModelConfig, DatasetConfig, TrainingConfig, LoRAConfig, HNMConfig
)
from finetune_embedding.services.pipeline import FineTuningService
from finetune_embedding.exceptions import (
    ConfigurationError, DataLoadingError, ModelError, TrainingError, EvaluationError, FineTuningError
)
...
def minimal_app_settings(tmp_path):
    ...
    return AppSettings(
        ...
        training=TrainingConfig(output_dir=output_dir, epochs=1, train_batch_size=2, eval_batch_size=2),
...
def mock_pipeline_dependencies(mocker, mock_dataset_dict, mock_dataset, mock_model, mock_loss, mock_evaluator, mock_trainer_wrapper):
    ...
    mocks = {
        ...
        "determine_device": mocker.patch("finetune_embedding.services.pipeline.determine_device", return_value=torch.device("cpu")),
        "update_precision_flags": mocker.patch("finetune_embedding.services.pipeline.update_precision_flags"),
        "save_config": mocker.patch("finetune_embedding.services.pipeline.FineTuningService._save_effective_config"), # Mock internal method too
        "load_raw_datasets": mocker.patch("finetune_embedding.services.pipeline.load_raw_datasets", return_value=mock_dataset_dict),
        "parse_column_rename_map": mocker.patch("finetune_embedding.services.pipeline.parse_column_rename_map", return_value=None),
        "apply_column_renaming": mocker.patch("finetune_embedding.services.pipeline.apply_column_renaming", return_value=mock_dataset_dict),
        "auto_split_dataset": mocker.patch("finetune_embedding.services.pipeline.auto_split_dataset", return_value=mock_dataset_dict),
        "select_and_limit_splits": mocker.patch("finetune_embedding.services.pipeline.select_and_limit_splits", return_value=DatasetContainer(train=mock_dataset, eval=mock_dataset, test=mock_dataset)),
        "initialize_model": mocker.patch("finetune_embedding.services.pipeline.initialize_model", return_value=mock_model),
        "apply_hard_negative_mining": mocker.patch("finetune_embedding.services.pipeline.apply_hard_negative_mining", return_value=(DatasetContainer(train=mock_dataset, eval=mock_dataset, test=mock_dataset), "triplet")),
        "add_lora_adapter": mocker.patch("finetune_embedding.services.pipeline.add_lora_adapter"),
        "create_loss_function": mocker.patch("finetune_embedding.services.pipeline.create_loss_function", return_value=mock_loss),
        "create_evaluator": mocker.patch("finetune_embedding.services.pipeline.create_evaluator", side_effect=[mock_evaluator, mock_evaluator]), # Dev then Test
        "TrainingWrapper": mocker.patch("finetune_embedding.services.pipeline.TrainingWrapper", return_value=mock_trainer_wrapper),
        "run_evaluation": mocker.patch("finetune_embedding.services.pipeline.run_evaluation"),
    }
    return mocks
...
def test_pipeline_run_minimal_success(minimal_app_settings, mock_pipeline_dependencies):
    ...
    # Check setup calls
    mock_pipeline_dependencies["set_seed"].assert_called_once_with(minimal_app_settings.training.seed)
    ...
    mock_pipeline_dependencies["save_config"].assert_called_once() # Check config saving
    ...
    mock_pipeline_dependencies["create_evaluator"].assert_has_calls([
        call(eval_dataset=ANY, effective_format=minimal_app_settings.dataset.dataset_format, dataset_config=minimal_app_settings.dataset, eval_batch_size=minimal_app_settings.training.eval_batch_size, name_prefix="eval"),
        call(eval_dataset=ANY, effective_format=minimal_app_settings.dataset.dataset_format, dataset_config=minimal_app_settings.dataset, eval_batch_size=minimal_app_settings.training.eval_batch_size, name_prefix="test"),
    ])

    # Check TrainingWrapper initialization and train call
    mock_pipeline_dependencies["TrainingWrapper"].assert_called_once()
    mock_pipeline_dependencies["TrainingWrapper"].return_value.train.assert_called_once()
    mock_pipeline_dependencies["TrainingWrapper"].return_value.save_final_model.assert_called_once()
...
def test_pipeline_run_with_hnm(minimal_app_settings, mock_pipeline_dependencies):
    ...
    assert mock_pipeline_dependencies["create_evaluator"].call_count == 2
    mock_pipeline_dependencies["create_evaluator"].assert_has_calls([
        call(eval_dataset=ANY, effective_format=effective_format_after_hnm, dataset_config=minimal_app_settings.dataset, eval_batch_size=minimal_app_settings.training.eval_batch_size, name_prefix="eval"),
        call(eval_dataset=ANY, effective_format=effective_format_after_hnm, dataset_config=minimal_app_settings.dataset, eval_batch_size=minimal_app_settings.training.eval_batch_size, name_prefix="test"),
    ])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("initialize_model", ModelError("Mock model init fail"), "Model Initialization"),
        ("apply_hard_negative_mining", FineTuningError("Mock HNM fail"), "Hard Negative Mining"),
        ...
        ("create_evaluator", EvaluationError("Mock dev eval fail"), "Evaluator Setup"), # First call fails
        ("TrainingWrapper", TrainingError("Mock training fail"), "Training"), # Mocking the class init fails
        ("run_evaluation", EvaluationError("Mock test eval fail"), "Test Evaluation"), # This configures run_evaluation to fail *if called*
    ]
)
def test_pipeline_error_propagation(minimal_app_settings, mock_pipeline_dependencies, failing_mock, error_type, stage_name):
    ...
    if stage_name == "Test Evaluation":
         ...
         mock_pipeline_dependencies["create_evaluator"].side_effect = [MagicMock(), EvaluationError("Mock test eval create fail")]
    ...
    if stage_name == "Training":
        ...
        assert mock_pipeline_dependencies["create_evaluator"].call_count >= 2 # Ensure it was attempted
        mock_pipeline_dependencies["run_evaluation"].assert_not_called() # Ensure the failing step wasn't reached
    else:
...
def test_pipeline_training_execution_error(minimal_app_settings, mock_pipeline_dependencies):
    """Test error during the trainer.train() execution."""
    # Make the train() method of the wrapper instance raise an error
    mock_pipeline_dependencies["TrainingWrapper"].return_value.train.side_effect = TrainingError("CUDA OOM")
    ...
    mock_pipeline_dependencies["TrainingWrapper"].return_value.save_final_model.assert_not_called()
...
def test_pipeline_unexpected_error(minimal_app_settings, mock_pipeline_dependencies):
    ...
    mock_pipeline_dependencies["initialize_model"].side_effect = ValueError("Unexpected value error")

    service = FineTuningService(minimal_app_settings)

    # Should be wrapped in FineTuningError
    with pytest.raises(FineTuningError, match="Unexpected pipeline error: Unexpected value error"):
        service.run_pipeline()
