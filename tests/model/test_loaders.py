...
def test_initialize_model_success(mock_st_class, mock_model_config, mock_training_config):
...
@patch('finetune_embedding.model.loaders.SentenceTransformer')
def test_initialize_model_precision(mock_st_class, use_fp16, use_bf16, expected_dtype, mock_model_config, mock_training_config):
...
@patch('finetune_embedding.model.loaders.SentenceTransformer', side_effect=OSError("Simulated loading error"))
def test_initialize_model_os_error(mock_st_class, mock_model_config, mock_training_config):
    """Test handling of OSError during model loading."""
    with pytest.raises(ModelError, match="Failed to load model: Simulated loading error"):
...
@patch('finetune_embedding.model.loaders.SentenceTransformer', side_effect=Exception("Some other error"))
def test_initialize_model_other_error(mock_st_class, mock_model_config, mock_training_config):
    """Test handling of generic exceptions during model loading."""
    # Expect the message from the generic except block
    with pytest.raises(ModelError, match="Model initialization failed: Some other error"):
        initialize_model(mock_model_config, mock_training_config, torch.device('cpu'))
    mock_st_class.assert_called_once() # Verify constructor was attempted
