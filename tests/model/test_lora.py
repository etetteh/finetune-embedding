...
class MockSentenceTransformer:
    def __init__(self):
        ...
        self.auto_model = MagicMock(name="auto_model_instance") # Give it a name for clarity
...
def test_add_lora_adapter_success(mocker, mock_model, valid_lora_settings):
    ...
    mock_peft_model = mocker.patch('peft.PeftModel') # Patch PeftModel from peft
    ...
    mock_model._first_module().auto_model.__class__ = MagicMock # Set class to a generic mock
...
def test_add_lora_adapter_peft_add_error(mocker, mock_model, valid_lora_settings):
    ...
    with pytest.raises(ModelError, match="Failed to add LoRA adapter: PEFT internal error"):
        add_lora_adapter(mock_model, valid_lora_settings)
...
def test_add_lora_adapter_already_peft_warning(mocker, mock_model, valid_lora_settings):
    ...
    mocker.patch('peft.PeftModel')
    mock_logger_warning = mocker.patch('finetune_embedding.model.lora.logger.warning') # Patch the logger
