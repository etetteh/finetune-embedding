...
from finetune_embedding.config.loaders import (
    load_and_validate_config, _load_config_from_file, _create_arg_parser, _structure_config_dict
)
from finetune_embedding.exceptions import ConfigurationError
from finetune_embedding.config.settings import AppSettings, DatasetFormat # Import for type check
...
def test_structure_config_dict_prefixed_args():
    ...
    flat_config = {
        "hnm_num_negatives": 5, "hnm_use_faiss": True,
        "lora_rank": 8, "use_lora": True, # Note: use_lora is not prefixed
        "model_name_or_path": "dummy", "dataset_format": "pair", "output_dir": "dummy"
    }
    structured = _structure_config_dict(flat_config)
...
def auto_mock_config_helpers(mocker):
     ...
     mocks = {
         'load_file': mocker.patch('finetune_embedding.config.loaders._load_config_from_file'),
         'load_dotenv': mocker.patch('finetune_embedding.config.loaders.load_dotenv'),
     }
     yield mocks

def test_load_validate_cli_only_minimal(auto_mock_argparse, auto_mock_config_helpers, mock_args_namespace):
...
def test_load_validate_config_file_override(auto_mock_argparse, auto_mock_config_helpers, sample_json_file_factory, minimal_config_dict, mock_args_namespace):
    ...
    auto_mock_argparse['parse_known_args'].return_value = (mock_known_args, []) # Use the mock dict
    auto_mock_argparse['parse_args'].return_value = mock_args_namespace
...
def test_load_validate_cli_overrides_config(auto_mock_argparse, auto_mock_config_helpers, sample_json_file_factory, minimal_config_dict, mock_args_namespace):
    ...
    auto_mock_argparse['parse_known_args'].return_value = (mock_known_args, remaining_cli) # Use the mock dict
    ...
    mock_args_namespace.dataset_name = "config-hub-dataset" # From config (not overridden by CLI)
...
@pytest.mark.parametrize("missing_arg", ["model_name_or_path", "dataset_format", "output_dir"])
def test_load_validate_missing_required_arg_fails(auto_mock_argparse, auto_mock_config_helpers, mock_args_namespace, missing_arg):
...
def test_load_validate_dataset_format_property(mocker, mock_args_namespace, ds_format):
    ...
    mock_load_file = mocker.patch('finetune_embedding.config.loaders._load_config_from_file')
