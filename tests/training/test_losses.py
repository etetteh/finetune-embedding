...
@pytest.mark.parametrize(
    "data_format, num_labels, expected_loss_type",
    [
        ...
        (
            ...
            2,
            losses.CoSENTLoss,
        ),  # Or CosineSimilarityLoss, AnglELoss depending on default
        ("pair-class", 2, losses.SoftmaxLoss),
        ("pair-class", 3, losses.SoftmaxLoss),
    ],
)
def test_create_loss_function_selection(
    mock_sentence_transformer_instance, data_format, num_labels, expected_loss_type
):
    """Test correct loss selection based on format."""
    loss = create_loss_function(
        mock_sentence_transformer_instance, data_format, num_labels
    )
...
def test_create_loss_function_softmax_invalid_labels(
...
def test_create_loss_function_softmax_no_emb_dim(mock_sentence_transformer_instance):
    ...
    mock_sentence_transformer_instance.get_sentence_embedding_dimension.return_value = (
        None
    )
