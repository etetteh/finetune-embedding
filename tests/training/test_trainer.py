...
def mock_evaluator():
    ...
    with (
        patch("finetune_embedding.training.trainer.TripletEvaluator", MagicMock),
        patch(
            "finetune_embedding.training.trainer.EmbeddingSimilarityEvaluator",
            MagicMock,
        ),
        patch(
            "finetune_embedding.training.trainer.BinaryClassificationEvaluator",
            MagicMock,
        ),
    ):
...
def test_create_training_args_basic(
    minimal_training_config, mock_model, mock_loss, mock_dataset
):
...
def test_create_training_args_with_evaluator(
    minimal_training_config, mock_model, mock_loss, mock_dataset, mock_evaluator
):
    ...
    assert (
        args.metric_for_best_model == "eval_my_data-dev_cosine_accuracy"
    )  # Assuming BinaryClassificationEvaluator mock
    assert args.greater_is_better is True


def test_create_training_args_with_evaluator_user_metric(
    minimal_training_config, mock_model, mock_loss, mock_dataset, mock_evaluator
):
    ...
    assert args.load_best_model_at_end is True
    assert (
        args.metric_for_best_model == "eval_custom_metric"
    )  # User metric overrides inference
    assert args.greater_is_better is True  # Default assumption


def test_create_training_args_eval_strategy_override(
    minimal_training_config, mock_model, mock_loss, mock_dataset
):
...
def test_create_training_args_missing_output_dir(
    minimal_training_config, mock_model, mock_loss, mock_dataset
):
...
def test_create_training_args_dir_creation_fails(
    minimal_training_config, mock_model, mock_loss, mock_dataset, mocker
):
    ...
    mock_makedirs.assert_called_once_with(
        minimal_training_config.output_dir, exist_ok=True
    )
...
def test_training_wrapper_train_success(
    mock_sbert_trainer,
...
@patch("finetune_embedding.training.trainer.SentenceTransformerTrainer")
def test_training_wrapper_train_handles_no_evaluator(
    mock_sbert_trainer, minimal_training_config, mock_model, mock_loss, mock_dataset
):
...
def test_training_wrapper_train_error_propagation(
    minimal_training_config, mock_model, mock_loss, mock_dataset
):
    """Test errors during prerequisite checks."""
    # No model
    wrapper_no_model = TrainingWrapper(
        model=None,
        loss=mock_loss,
        train_dataset=mock_dataset,
        training_args_config=minimal_training_config,
    ...
    # No train dataset
    wrapper_no_data = TrainingWrapper(
        model=mock_model,
        loss=mock_loss,
        train_dataset=None,
        training_args_config=minimal_training_config,
    ...
    # No loss
    wrapper_no_loss = TrainingWrapper(
        model=mock_model,
        loss=None,
        train_dataset=mock_dataset,
        training_args_config=minimal_training_config,
...
@patch("finetune_embedding.training.trainer.SentenceTransformerTrainer")
def test_training_wrapper_train_execution_error(
    mock_sbert_trainer, minimal_training_config, mock_model, mock_loss, mock_dataset
):
...
def test_save_final_model_success(
    ...
    expected_save_path = os.path.join(
        str(minimal_training_config.output_dir), "final_model"
    )
...
def test_save_final_model_no_trainer(
    minimal_training_config, mock_model, mock_loss, mock_dataset
):
...
def test_save_final_model_save_exception(
    minimal_training_config, mock_model, mock_loss, mock_dataset, mocker
):
