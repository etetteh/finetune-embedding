...
def apply_hard_negative_mining(
    ...
    if dataset_config.dataset_format != "pair":
        logger.warning(f"Attempted HNM but initial format is '{dataset_config.dataset_format}', not 'pair'. Skipping.")
    ...
    if not datasets.train_dataset:
         raise DataLoadingError("Training data must be loaded before Hard Negative Mining.")
    ...
    try:
        ...
        positive_col = initial_pair_columns["sentence2"]
         ...
         raise ConfigurationError(f"Could not determine input columns for HNM: {e}") from e
    ...
    logger.debug(f"HNM arguments: { {k:v for k,v in hnm_args.items() if k != 'model'} }")
    ...
    try:
        ...
        new_datasets.train_dataset = mine_hard_negatives(dataset=datasets.train_dataset, **hnm_args)
        mined_len = len(new_datasets.train_dataset)
        logger.info(f"Training set after HNM: {mined_len} triplets (from {original_len} pairs).")
        if mined_len == 0:
            raise DataLoadingError("Hard Negative Mining resulted in an empty training set.")
        ...
        if datasets.eval_dataset and len(datasets.eval_dataset) > 0:
            ...
            new_datasets.eval_dataset = mine_hard_negatives(dataset=datasets.eval_dataset, **hnm_args)
            mined_len = len(new_datasets.eval_dataset)
            logger.info(f"Evaluation set after HNM: {mined_len} triplets (from {original_len} pairs).")
            if mined_len == 0: new_datasets.eval_dataset = None
        else:
        ...
        if datasets.test_dataset and len(datasets.test_dataset) > 0:
            ...
            new_datasets.test_dataset = mine_hard_negatives(dataset=datasets.test_dataset, **hnm_args)
            mined_len = len(new_datasets.test_dataset)
            logger.info(f"Test set after HNM: {mined_len} triplets (from {original_len} pairs).")
            if mined_len == 0: new_datasets.test_dataset = None
        else:
            logger.info("No test set provided or empty, skipping HNM for test.")
            new_datasets.test_dataset = None

        effective_format = "triplet"
        logger.info(f"Hard Negative Mining completed. Effective format is now '{effective_format}'.")
