...
class DatasetContainer:
    """Simple container to hold train, eval, and test datasets."""
    def __init__(self, train: Optional[Dataset] = None, eval: Optional[Dataset] = None, test: Optional[Dataset] = None):
...
def load_raw_datasets(config: DatasetConfig, cache_dir: Optional[str]) -> DatasetDict:
    ...
    if config.dataset_name and not config.file_format and not config.data_files and not config.data_dir:
        logger.info(f"Loading dataset '{config.dataset_name}' from Hugging Face Hub.")
        load_path = config.dataset_name
        if config.dataset_config_name: load_kwargs["name"] = config.dataset_config_name
    elif config.file_format or (config.dataset_name and (config.data_files or config.data_dir)):
        format_type = config.file_format or config.dataset_name
        if not format_type:
             raise DataLoadingError("Cannot determine local file format. Set --file_format or use --dataset_name as format type.")
        ...
        if config.data_files:
            ...
            raise DataLoadingError(f"Loading format '{format_type}' requires --data_files or --data_dir.")
    else:
        raise DataLoadingError("Invalid dataset source configuration provided.")

    logger.info(f"Calling load_dataset(path='{load_path}', split=None, **{load_args}, **{load_kwargs})")
    try:
        ...
        if isinstance(dataset, Dataset):
            logger.info(f"Loaded single Dataset ({len(dataset)} examples). Treating as 'train' initially.")
        ...
        logger.error(f"Dataset loading failed: File/Dataset not found. Check paths/names. Details: {e}", exc_info=True)
        raise DataLoadingError(f"Dataset file or path not found: {load_path or config.data_files or config.data_dir}") from e
    except Exception as e:
        logger.error(f"Dataset loading failed unexpectedly. Check arguments/source. Details: {e}", exc_info=True)
