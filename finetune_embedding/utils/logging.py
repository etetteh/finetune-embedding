...
def setup_logging(log_level_str: str, log_file: Optional[str] = None) -> None:
    ...
    if log_file:
        try:
            ...
            logging.error(
                f"Failed to create log directory for {log_file}: {e}. Logging to console only.",
                exc_info=False,
            )
    ...
    app_logger.info(
        f"Logging configured at level {log_level_str.upper()}."
        + (f" Logging to file: {log_file}" if log_file else "")
    )
