...
def determine_device() -> torch.device:
    ...
    if torch.cuda.is_available():
        ...
        try:
            ...
            logger.info("CUDA not available, but MPS (Apple Silicon GPU) is available and functional. Using device: mps")
        except Exception as e:
            logger.warning(f"MPS available but failed test ({e}). Falling back to CPU.")
            device = torch.device("cpu")
    else:
        device = torch.device("cpu")
        logger.warning("CUDA and MPS are not available. Using CPU. Training will be significantly slower.")

    logger.info(f"Selected device: {device}")
    return device

def update_precision_flags(training_config: TrainingConfig, device: torch.device) -> None:
    """
    ...
    if training_config.use_fp16 and training_config.use_bf16:
        logger.warning("Both use_fp16 and use_bf16 requested. Prioritizing BF16 if supported, otherwise FP16.")
        if bf16_supported:
            training_config.use_fp16 = False # Prioritize BF16
            logger.info("BF16 supported and prioritized. Disabling FP16 flag.")
        else:
             training_config.use_bf16 = False # BF16 not supported, keep FP16 (will be checked next)
             logger.info("BF16 not supported by device. Keeping FP16 flag (will be checked).")
    ...
    if training_config.use_bf16 and not bf16_supported:
        logger.warning(f"BF16 requested but not supported by the device ({device.type}). Disabling BF16.")
        training_config.use_bf16 = False

    logger.info(f"Final precision flags: use_fp16={training_config.use_fp16}, use_bf16={training_config.use_bf16}")
