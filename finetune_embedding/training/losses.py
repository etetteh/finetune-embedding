...
def create_loss_function(
    ...
    except ConfigurationError as ce: # Catch specific config errors raised intentionally above
        logger.error(f"Configuration error creating loss function: {ce}", exc_info=False)
    ...
    # Catch other potential ValueErrors (e.g., from SentenceTransformer methods if they raise it)
        ...
        raise ConfigurationError(f"Loss creation failed due to value error: {ve}") from ve
        ...
        raise ModelError(f"Unexpected failure during loss function creation: {e}") from e
