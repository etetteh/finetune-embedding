...
from finetune_embedding.config.settings import TrainingConfig, DEFAULT_EVAL_SPLIT
...
class TrainingWrapper:
    ...
    def _create_training_args(self) -> SentenceTransformerTrainingArguments:
        ...
        try:
            ...
            raise ConfigurationError(f"Failed to create output directory '{output_dir}': {e}") from e

        run_name = self.training_args_config.run_name or os.path.basename(str(output_dir).rstrip(os.sep)) or "sbert-finetune-run"
        ...
        if self.training_args_config.eval_strategy != "no" and self.dev_evaluator and not metric_for_best_model:
            ...
            if isinstance(self.dev_evaluator, TripletEvaluator):
                ...
                metric_for_best_model = f"{eval_base_name}_cosine_similarity" # Spearman is common
            elif isinstance(self.dev_evaluator, BinaryClassificationEvaluator):
                metric_for_best_model = f"{eval_base_name}_cosine_accuracy"
            else:
                logger.warning("Could not infer default metric for best model from evaluator type.")

            if metric_for_best_model:
                 logger.info(f"Defaulting metric_for_best_model to '{metric_for_best_model}' (greater_is_better={greater_is_better})")
        ...
        if effective_eval_strategy != "no" and not self.dev_evaluator:
            logger.warning(f"Evaluation strategy is '{effective_eval_strategy}' but no dev evaluator available. Changing strategy to 'no'.")
        ...
        args = SentenceTransformerTrainingArguments(
            ...
            eval_steps=self.training_args_config.eval_steps if effective_eval_strategy == "steps" else None,
            save_strategy=self.training_args_config.save_strategy,
            save_steps=self.training_args_config.save_steps if self.training_args_config.save_strategy == "steps" else None,
            save_total_limit=self.training_args_config.save_limit,
            logging_strategy=self.training_args_config.logging_strategy,
            logging_steps=self.training_args_config.logging_steps if self.training_args_config.logging_strategy == "steps" else None,
            ...
            run_name=run_name,
            load_best_model_at_end=bool(metric_for_best_model and self.dev_evaluator), # Only load best if metric exists AND evaluator exists
            metric_for_best_model=metric_for_best_model if self.dev_evaluator else None, # Only set metric if evaluator exists
    ...
    def train(self) -> None:
        ...
        if not self.model: raise TrainingError("Model not initialized.")
        if not self.train_dataset: raise TrainingError("Train dataset not loaded.")
        if not self.loss: raise TrainingError("Loss function not created.")

        training_args = self._create_training_args() # This now handles the eval_strategy override
        ...
        try:
            ...
            if "OutOfMemoryError" in str(e) or "CUDA out of memory" in str(e):
                 logger.error("Suggestion: Reduce batch sizes or increase gradient_accumulation_steps.")
    ...
    def save_final_model(self) -> str:
        ...
        final_save_path = os.path.join(str(self.training_args.output_dir), "final_model")
