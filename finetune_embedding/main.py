...
try:
    ...
    from finetune_embedding.exceptions import FineTuningError, ConfigurationError, DataLoadingError
    from finetune_embedding.utils.logging import setup_logging # Import the setup function
except ImportError as e:
     logger.critical(f"Failed to import necessary modules. Ensure the package structure is correct and dependencies are installed. Error: {e}", exc_info=True)
     print(f"Import Error: {e}. Check installation and package structure.", file=sys.stderr)
     sys.exit(1)
...
def main() -> int:
    ...
    try:
        ...
        current_logger = logging.getLogger("finetune_embedding") if settings else logger
        current_logger.critical(f"An unexpected critical error occurred: {e}", exc_info=True)
        exit_code = 1
    finally:
        logging.shutdown()
