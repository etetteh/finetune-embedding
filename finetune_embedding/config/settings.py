...
from pydantic import ( # Group imports
    BaseModel, Field, field_validator, FilePath, DirectoryPath, Json, ConfigDict
)
...
class ModelConfig(BaseModel):
    model_name_or_path: str = Field(..., description="Path/name of the pre-trained model.")
    language: str = Field("en", description="Model language for metadata.")
    license_type: str = Field("apache-2.0", description="Model license.")
    trust_remote_code: bool = Field(False, description="Allow loading models with custom code.")
    cache_dir: Optional[Path] = Field(None, description="Directory for caching datasets and models.")

class DatasetConfig(BaseModel):
    dataset_name: Optional[str] = Field(None, description="Name/path of the dataset (Hub ID or local path type like 'csv').")
    dataset_config_name: Optional[str] = Field(None, description="Specific configuration name for Hub datasets.")
    dataset_format: DatasetFormat = Field(..., description="Logical format of the *initial* dataset.")
    file_format: Optional[Literal["csv", "json", "text", "xml", "parquet"]] = Field(None, description="Explicit format of local data files.")
    data_files: Optional[Union[List[Union[str, Path]], Dict[str, Union[str, Path, List[Union[str, Path]]]]]] = Field(None, description="Path(s) to local data files/glob patterns or dict mapping splits to files.")
    data_dir: Optional[Path] = Field(None, description="Path to directory containing local data files.")
    column_rename_map: Optional[Union[Json, Path, Dict[str, str]]] = Field(None, description="JSON string, path to JSON file, or dict mapping original to standard column names.")
    train_split: str = Field(DEFAULT_TRAIN_SPLIT, description="Name of the training split.")
    eval_split: str = Field(DEFAULT_EVAL_SPLIT, description="Name of the evaluation split.")
    test_split: str = Field(DEFAULT_TEST_SPLIT, description="Name of the test split.")
    train_limit: int = Field(0, ge=0, description="Max examples for training (0 for all).")
    eval_limit: int = Field(0, ge=0, description="Max examples for evaluation (0 for all).")
    test_limit: int = Field(0, ge=0, description="Max examples for testing (0 for all).")
    num_labels: int = Field(2, gt=0, description="Number of classes for 'pair-class' datasets. Used for SoftmaxLoss.")
...
class TrainingConfig(BaseModel):
    ...
    output_dir: Path = Field(..., description="Directory to save checkpoints and final model.")
    ...
    def check_torch_compile_availability(cls, v):
        ...
        if v and not hasattr(torch, "compile"):
            logging.warning("torch.compile requested but not available (requires PyTorch 2.0+). Disabling.")
            return False
        return v
